...
while ($j < $no_of_target_words){
	if($flag_list[$j] == 1){
		...
		for($i = 0; $i < $win_size; $i = $i + 1){
			...
			#print "$maxsyn2:";
			#print "$wps2gloss->[0]\n";
			#push(@freq_list,$maxsyn1);
			#push(@freq_list,$maxsyn2);
			#push(@output,$maxsyn1);
			#push(@output,$maxsyn2);
			#push(@output,$maxval);
			#push(@output,$win_size);
...
#print "freq list size =$frq_list_size \n";
for($a = 0; $a < $frq_list_size; $a = $a + 1) {
	$b = first_index($freq_list[$a],@freq_list);
	#print "$freq_list[$a]=$b\n";
	
	if($a eq $b){
		$freq_cnt = count($freq_list[$a],@freq_list);
#		print "$freq_list[$a]: freq cnt = $freq_cnt \t first_appear = $b,$a\n";
	}
}
